...
const ThreeDControls: React.FC<ThreeDControlsProps> = ({ isBuildMode, setIsBuildMode, resetCamera }) => {
    return (
        <div className="absolute top-[80px] left-4 bg-zinc-900 bg-opacity-80 backdrop-blur-sm p-3 rounded-lg shadow-lg border border-zinc-800 z-20">
            <h3 className="text-white font-bold text-lg mb-3 border-b border-zinc-700 pb-2 flex items-center gap-2">
            ...
            <div className="flex flex-col gap-2">
                <label className="flex items-center justify-between cursor-pointer">
                    <span className="text-zinc-300 text-sm font-medium">Byggläge</span>
                    <div
                        className={`relative w-11 h-6 rounded-full transition-all duration-300 ${isBuildMode ? 'bg-purple-600' : 'bg-zinc-600'}`}
                ...
                <button
                    onClick={resetCamera}
                    className="w-full bg-zinc-700 hover:bg-zinc-600 text-zinc-200 font-semibold py-2 px-3 rounded-md text-sm transition-colors flex items-center justify-center gap-2"
                >
